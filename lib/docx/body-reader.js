...
import {readMathElement} from "./math";
...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                fontSize: fontSize,
                color: element.firstOrEmpty("w:color").attributes["w:val"], // added color
    ...
    var xmlElementReaders = {
        ...
        "m:oMathPara": readMathElements,
        "m:oMath": readMathElements
    };
    ...
    function readTableRow(element) {
        ...
        var height = dxaToPx(properties.firstOrEmpty("w:trHeight").attributes["w:val"]);
        if (height == "NaNpx") height = "17px";
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            cell.width = dxaToPx(properties.children[0].attributes["w:w"]);
            cell.alignment = getTextAlign(element);
            return cell;
        });
    }
    ...
    function readBlip(element, blip) {
        ...
        // convert gdoc image to mathml
        if (properties.descr && properties.descr.startsWith("<math"))
        	return elementResult(documents.Html(properties.descr));
        var extent = element.first("wp:extent").attributes;
        var altText = isBlank(properties.descr) ? properties.title : properties.descr;
        return readImage(findBlipImageFile(blip), {
        	altText: altText,
        	width: emuToPx(extent["cx"]),
        	height: emuToPx(extent["cy"])
        });
    }
    ...
    function readMathElements(element) {
    	var str = readMathElement(element);
    	return elementResult(documents.Html(str));
    }
    ...
    function readImage(imageFile, properties) {
...
function dxaToPx(dxaValue) {
	return Math.round(dxaValue/20) + "px";
};

function emuToPx(emu) {
	return Math.round(emu/914400 * 72) + "px";
}

function getTextAlign(element) {
    var pPr = element.firstOrEmpty("w:p").firstOrEmpty("w:pPr");
    return (pPr.children) ? pPr.firstOrEmpty("w:jc").attributes["w:val"] : null;
}
